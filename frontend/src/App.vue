...
import { api, type AiDraft, type BatchReadiness, type DatasetDefinition, type MetricResult, type Perspective, type PnlResult, type PvmResult } from './api'

type View = 'overview' | 'import' | 'result' | 'pvm' | 'tree' | 'ai'
...
const title = computed(() => ({ overview: '经营总览', import: '数据批次', result: '损益计算结果', pvm: 'PVM因素分析', tree: '指标树', ai: 'AI经营分析' }[view.value]))
...
function loadDemo() { source.value = 'demo'; metrics.value = demoMetrics; results.value = demoResults; pvmRows.value = demoPvm; treeRows.value = demoMetrics; aiDraft.value = { ...demoAi, batchNo: batchNo.value }; readiness.value = { batchNo: batchNo.value, perspective: perspective.value, readyForCalculation: true, datasets: [{ datasetCode: 'INC_COST', datasetName: '收入成本明细', required: true, present: true, status: 'VALID', issueCount: 0 }, { datasetCode: 'BUDGET', datasetName: '年度预算收入成本', required: true, present: true, status: 'VALID', issueCount: 0 }] }; datasets.value = [{ code: 'BASE_ACT_INC_COST', name: '基地实际收入成本明细', required: true, fields: [] }, { code: 'BASE_BUD_INC_COST', name: '基地年度预算收入成本', required: true, fields: [] }] }
async function load() {
  loading.value = true; error.value = ''
  try {
    if (view.value !== 'import') metrics.value = await api.overview(period.value, perspective.value, scope.value)
    readiness.value = await api.readiness(batchNo.value); datasets.value = await api.datasets(perspective.value); source.value = 'api'
...
function pickFile(event: Event) { const input = event.target as HTMLInputElement; selectedFile.value = input.files && input.files.length ? input.files[0] : null }
async function submitImport() {
  if (!selectedDataset.value || !selectedFile.value) { importMessage.value = '请选择数据集并上传 Excel 文件。'; return }
  try { await api.createBatch({ batchNo: batchNo.value, period: period.value, perspective: perspective.value }); } catch { /* 已存在时继续使用当前批次 */ }
  try { const task = await api.createImport(batchNo.value, { datasetCode: selectedDataset.value, fileName: selectedFile.value.name }); await api.validateImport(task.taskNo, selectedFile.value); importMessage.value = `校验完成：${selectedFile.value.name}，请刷新批次完整性。`; await load() } catch (e) { importMessage.value = e instanceof Error ? e.message : '导入失败，已保留当前演示数据。' }
}
...
<template>
  <main class="app-shell">
    ...
    <div class="layout">
      <aside class="sidebar"><div class="side-title">平台工作台</div><button v-for="item in ([['overview','经营总览'],['import','数据批次'],['result','损益结果'],['pvm','PVM因素分析'],['tree','指标树'],['ai','AI经营分析']] as [View,string][])" :key="item[0]" :class="['nav-item', { active: view === item[0] }]" @click="view = item[0]; load()">{{ item[1] }}</button><div class="side-note">当前用户<br /><b>{{ currentUser }}</b><br /><span>{{ source === 'api' ? '已连接后端' : '脱敏演示模式' }}</span></div></aside>
      <section class="content"><div class="toolbar"><label>期间<select v-model="period"><option>2026-08</option><option>2026-07</option><option>2026-06</option></select></label><label>视角<select v-model="perspective"><option value="BASE">基地视角</option><option value="PRODUCT_LINE">产品线视角</option><option value="BUSINESS_UNIT">事业部视角</option></select></label><label>批次<input v-model="batchNo" /></label><label>范围<select v-model="scope"><option>ALL</option><option>义乌基地</option><option>泰国基地</option></select></label><button class="primary" :disabled="loading" @click="load">{{ loading ? '加载中…' : '刷新分析' }}</button></div>
        ...
        <template v-else-if="view === 'import'"><section class="panel import-panel"><div class="panel-head"><h3>月度数据批次导入</h3><span>收入成本、费用、预算等数据分别上传；校验通过后才可计算</span></div><div class="import-form"><label>数据集<select v-model="selectedDataset"><option value="">请选择数据集</option><option v-for="d in datasets" :key="d.code" :value="d.code">{{ d.name }}{{ d.required ? ' · 必需' : '' }}</option></select></label><label>Excel文件<input type="file" accept=".xlsx,.xls" @change="pickFile" /></label><button class="primary" @click="submitImport">上传并校验</button></div><div v-if="importMessage" class="notice">{{ importMessage }}</div><div class="dataset-list"><div v-for="row in readiness?.datasets" :key="row.datasetCode" class="dataset-row"><span>{{ row.datasetName }}</span><b :class="row.status === 'VALID' ? 'positive' : 'negative'">{{ row.status }}</b><small>{{ row.issueCount ? `${row.issueCount} 个问题` : '无问题' }}</small></div></div></section></template>
...
.import-form { display:flex; align-items:end; flex-wrap:wrap; gap:16px; padding:15px; background:#fafbfc; border:1px solid #edf0f3; border-radius:6px; } .import-form input[type=file] { min-width:260px; } .dataset-list { margin-top:20px; border-top:1px solid #edf0f3; } .dataset-row { display:grid; grid-template-columns:1fr 100px 100px; gap:12px; padding:12px 4px; border-bottom:1px solid #edf0f3; font-size:13px; } .dataset-row small { color:#8b96a4; }
